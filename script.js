...
let utenti = [
    { id: 1, nome: 'Admin', email: 'admin@fenix.it', ruolo: 'admin', stato: 'attivo', ultimoAccesso: '26/11/2025 10:30' },
    { id: 2, nome: 'Operatore', email: 'operatore@fenix.it', ruolo: 'operatore', stato: 'attivo', ultimoAccesso: '25/11/2025 18:45' },
    { id: 3, nome: 'Mario Rossi', email: 'mario@fenix.it', ruolo: 'viewer', stato: 'disattivo', ultimoAccesso: '20/11/2025 09:00' }
];
...
function loadUtenti() {
    ...
    const ruoloBadge = { 'admin': 'bg-danger', 'operatore': 'bg-info', 'viewer': 'bg-secondary' };
    const ruoloLabel = { 'admin': 'Admin', 'operatore': 'Operatore', 'viewer': 'Viewer' };
    
    tbody.innerHTML = utenti.map(u => `
        <tr>
            <td><strong>${u.nome}</strong></td>
            <td>${u.email}</td>
            <td><span class="badge ${ruoloBadge[u.ruolo]}">${ruoloLabel[u.ruolo]}</span></td>
            <td><span class="badge ${u.stato === 'attivo' ? 'bg-success' : 'bg-secondary'}">${u.stato === 'attivo' ? 'Attivo' : 'Disattivo'}</span></td>
            <td><small class="text-muted">${u.ultimoAccesso}</small></td>
            <td class="text-end">
                <button class="btn btn-sm btn-outline-primary me-1" onclick="modificaUtente(${u.id})"><i class="bi bi-pencil"></i></button>
                ${u.ruolo !== 'admin' ? `<button class="btn btn-sm btn-outline-danger" onclick="eliminaUtente(${u.id})"><i class="bi bi-trash"></i></button>` : ''}
            </td>
        </tr>
    `).join('');
}
...
function nuovoUtente() {
    ...
    new bootstrap.Modal(document.getElementById('modalUtente')).show();
}
...
function modificaUtente(id) {
    ...
    document.getElementById('utenteRuolo').value = utente.ruolo;
    document.getElementById('utenteStato').value = utente.stato;
    document.getElementById('utentePassword').value = '';
...
function salvaUtente() {
    ...
    const stato = document.getElementById('utenteStato').value;
    ...
    if (id) {
        // Modifica
        const utente = utenti.find(u => u.id === parseInt(id));
        if (utente) {
            utente.nome = nome;
            utente.email = email;
            utente.ruolo = ruolo;
        ...
        // Nuovo
        utenti.push({
            id: Math.max(...utenti.map(u => u.id)) + 1,
    ...
    bootstrap.Modal.getInstance(document.getElementById('modalUtente')).hide();
    loadUtenti();
}
...
function eseguiBackup() {
    ...
    statusDiv.innerHTML = '<div class="alert alert-info"><i class="bi bi-hourglass-split"></i> Creazione backup in corso...</div>';
    ...
    if (document.getElementById('backupSpedizioni')?.checked) {
        backup.contenuto.spedizioni = spedizioni;
    }
    if (document.getElementById('backupRubrica')?.checked) {
        backup.contenuto.rubrica = rubrica;
    }
    if (document.getElementById('backupDistinte')?.checked) {
        backup.contenuto.distinte = distinte;
    }
    if (document.getElementById('backupConfig')?.checked) {
        backup.contenuto.config = {
            tariffe: tariffe,
            utenti: utenti
        };
    }
    ...
    a.href = url;
    a.download = `fenix-backup-${dataStr}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    
    statusDiv.innerHTML = '<div class="alert alert-success"><i class="bi bi-check-circle"></i> Backup creato e scaricato!</div>';
    showToast('Backup creato con successo', 'success');
    
    // Aggiungi allo storico
    aggiungiBackupStorico(dataStr, backup);
}

function ripristinaBackup() {
    const fileInput = document.getElementById('fileBackupInput');
    const statusDiv = document.getElementById('ripristinoStatus');
    
    if (!fileInput.files || fileInput.files.length === 0) {
        showToast('Seleziona un file di backup', 'warning');
        return;
    }
    ...
    statusDiv.innerHTML = '<div class="alert alert-info"><i class="bi bi-hourglass-split"></i> Lettura backup in corso...</div>';
    
    reader.onload = function(e) {
        try {
            const backup = JSON.parse(e.target.result);
            
            if (!backup.version || !backup.contenuto) {
                throw new Error('Formato backup non valido');
            }
            
            const sovrascriviDati = document.getElementById('backupSovrascriviDati')?.checked;
            let importati = [];
            
            if (backup.contenuto.spedizioni) {
                if (sovrascriviDati) {
                    spedizioni = backup.contenuto.spedizioni;
                } else {
                    spedizioni = [...spedizioni, ...backup.contenuto.spedizioni];
                }
                importati.push('spedizioni');
            }
            
            if (backup.contenuto.rubrica) {
                if (sovrascriviDati) {
                    rubrica = backup.contenuto.rubrica;
                } else {
                    rubrica = [...rubrica, ...backup.contenuto.rubrica];
                }
                importati.push('rubrica');
            }
            
            if (backup.contenuto.distinte) {
                if (sovrascriviDati) {
                    distinte = backup.contenuto.distinte;
                } else {
                    distinte = [...distinte, ...backup.contenuto.distinte];
                }
                importati.push('distinte');
            }
            
            if (backup.contenuto.config) {
                if (backup.contenuto.config.utenti) {
                    utenti = backup.contenuto.config.utenti;
                }
                importati.push('configurazioni');
            }
            
            statusDiv.innerHTML = `<div class="alert alert-success">
                <i class="bi bi-check-circle"></i> Backup ripristinato!<br>
                <small>Importati: ${importati.join(', ')}</small>
            </div>`;
            ...
            statusDiv.innerHTML = `<div class="alert alert-danger">
                <i class="bi bi-x-circle"></i> Errore: ${err.message}
            </div>`;
...
function aggiungiBackupStorico(dataStr, backup) {
    const size = new Blob([JSON.stringify(backup)]).size;
    storicoBackup.unshift({
        data: dataStr,
        dimensione: formatBytes(size),
        tipo: 'Manuale'
    });
...
function renderStoricoBackup() {
    const tbody = document.getElementById('storicoBackupBody');
    if (!tbody) return;
    
    if (storicoBackup.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">Nessun backup salvato</td></tr>';
        return;
    }
    
    tbody.innerHTML = storicoBackup.map((b, idx) => `
        <tr>
            <td>${b.data}</td>
            <td>${b.dimensione}</td>
            <td><span class="badge bg-info">${b.tipo}</span></td>
            <td><button class="btn btn-sm btn-outline-danger" onclick="eliminaBackupStorico(${idx})"><i class="bi bi-trash"></i></button></td>
        </tr>
    `).join('');
}

function eliminaBackupStorico(idx) {
    storicoBackup.splice(idx, 1);
    renderStoricoBackup();
}
...
let contrassegniFilter = 'tutti';

function filterContrassegni(filtro) {
    contrassegniFilter = filtro;
    
    // Aggiorna tab attivo
    document.querySelectorAll('#contrassegniTabs .nav-link').forEach(tab => {
...
// Override della funzione loadContrassegni esistente
const originalLoadContrassegni = typeof loadContrassegni === 'function' ? loadContrassegni : null;

loadContrassegni = function() {
    const contrassegniData = spedizioni.filter(s => s.dettagli.contrassegno > 0);
    
    let filteredData = contrassegniData;
    
    switch(contrassegniFilter) {
        case 'attesa':
            filteredData = contrassegniData.filter(s => 
                ['In Lavorazione', 'Spedito', 'In Transito', 'In Consegna'].includes(s.stato)
            );
            break;
        case 'incassati':
            filteredData = contrassegniData.filter(s => s.stato === 'Consegnato' && s.incassato);
            break;
        case 'scaduti':
            filteredData = contrassegniData.filter(s => s.stato === 'Giacenza');
            break;
    }
    
    // Calcola totali per le card
    const daIncassare = contrassegniData.filter(s => ['In Lavorazione', 'Spedito', 'In Transito', 'In Consegna'].includes(s.stato));
    const inTransito = contrassegniData.filter(s => ['In Transito', 'In Consegna'].includes(s.stato));
    const incassati = contrassegniData.filter(s => s.stato === 'Consegnato');
    const scaduti = contrassegniData.filter(s => s.stato === 'Giacenza');
    
    $('#contrassegniDaIncassare').text(daIncassare.reduce((sum, s) => sum + s.dettagli.contrassegno, 0).toFixed(2));
    $('#contrassegniDaIncassareCount').text(daIncassare.length);
    $('#contrassegniInTransito').text(inTransito.reduce((sum, s) => sum + s.dettagli.contrassegno, 0).toFixed(2));
    $('#contrassegniInTransitoCount').text(inTransito.length);
    $('#contrassegniIncassati').text(incassati.reduce((sum, s) => sum + s.dettagli.contrassegno, 0).toFixed(2));
    $('#contrassegniIncassatiCount').text(incassati.length);
    $('#contrassegniScaduti').text(scaduti.reduce((sum, s) => sum + s.dettagli.contrassegno, 0).toFixed(2));
    $('#contrassegniScadutiCount').text(scaduti.length);
    
    // Renderizza tabella
    const tbody = document.getElementById('contrassegniBody');
    if (!tbody) return;
    
    if (filteredData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">Nessun contrassegno trovato</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = filteredData.map(s => `
        <tr>
            ...
            <td>${s.data}</td>
            <td>${s.destinatario.nome}</td>
            <td><span class="badge-courier ${getBadgeClass(s.corriere)}">${s.corriere}</span></td>
            <td><strong class="text-success">€ ${s.dettagli.contrassegno.toFixed(2)}</strong></td>
            <td><span class="badge ${getStatoClass(s.stato)}">${s.stato}</span></td>
            <td>${s.incassato ? '<span class="badge bg-success">Incassato</span>' : '<span class="badge bg-warning text-dark">In Attesa</span>'}</td>
            <td class="text-end">
                ${!s.incassato && s.stato === 'Consegnato' ? `<button class="btn btn-sm btn-success" onclick="apriModalIncasso(${s.id})"><i class="bi bi-cash-coin"></i></button>` : ''}
                <button class="btn btn-sm btn-outline-primary" onclick="visualizzaDettaglio(${s.id})"><i class="bi bi-eye"></i></button>
            </td>
        </tr>
    `).join('');
};

function apriModalIncasso(id) {
    const sped = spedizioni.find(s => s.id === id);
    if (!sped) return;
    
    document.getElementById('incassoSpedId').textContent = '#' + id;
    document.getElementById('incassoImporto').textContent = sped.dettagli.contrassegno.toFixed(2);
    document.getElementById('incassoData').value = new Date().toISOString().split('T')[0];
    document.getElementById('incassoNote').value = '';
    
    new bootstrap.Modal(document.getElementById('modalIncasso')).show();
}

function esportaContrassegni() {
    showToast('Esportazione contrassegni in corso...', 'info');
    // Implementazione export CSV
    setTimeout(() => showToast('File esportato!', 'success'), 1000);
}

function riconciliaContrassegni() {
    showToast('Riconciliazione avviata...', 'info');
}
...
let ordiniDuplicati = [];
let ordiniDaUnificare = [];

// Simula ordini Shopify
let ordiniShopify = [
    { id: 'SH-1001', cliente: 'Mario Rossi', email: 'mario@email.it', indirizzo: 'Via Roma 123', cap: '00100', citta: 'Roma', telefono: '3331234567', prodotti: ['Prodotto A x2'], contrassegno: 89.90, data: '26/11/2025' },
    { id: 'SH-1002', cliente: 'Mario Rossi', email: 'mario@email.it', indirizzo: 'Via Roma 123', cap: '00100', citta: 'Roma', telefono: '3331234567', prodotti: ['Prodotto B x1'], contrassegno: 45.00, data: '26/11/2025' },
    { id: 'SH-1003', cliente: 'Mario Rossi', email: 'mario@email.it', indirizzo: 'Via Roma 123', cap: '00100', citta: 'Roma', telefono: '3331234567', prodotti: ['Prodotto C x3'], contrassegno: 120.50, data: '26/11/2025' },
    { id: 'SH-1010', cliente: 'Luigi Verdi', email: 'luigi@test.it', indirizzo: 'Via Milano 45', cap: '20100', citta: 'Milano', telefono: '3339876543', prodotti: ['Prodotto D x1'], contrassegno: 55.00, data: '25/11/2025' },
    { id: 'SH-1011', cliente: 'Luigi Verdi', email: 'luigi@test.it', indirizzo: 'Via Milano 45', cap: '20100', citta: 'Milano', telefono: '3339876543', prodotti: ['Prodotto E x2'], contrassegno: 78.00, data: '25/11/2025' },
    { id: 'SH-1020', cliente: 'Anna Bianchi', email: 'anna@mail.com', indirizzo: 'Corso Italia 78', cap: '10100', citta: 'Torino', telefono: '3355556666', prodotti: ['Prodotto F x1'], contrassegno: 35.00, data: '24/11/2025' }
];

function cercaDuplicati() {
    // Raggruppa ordini per cliente+indirizzo
    const gruppi = {};
    
    ordiniShopify.forEach(ordine => {
        const chiave = `${ordine.cliente.toLowerCase()}_${ordine.indirizzo.toLowerCase()}_${ordine.cap}`;
        if (!gruppi[chiave]) {
            gruppi[chiave] = [];
        }
    ...
    // Filtra solo i gruppi con più di un ordine (duplicati)
    ordiniDuplicati = Object.values(gruppi).filter(gruppo => gruppo.length > 1);
    
    return ordiniDuplicati;
}

function loadOrdiniDuplicati() {
    const duplicati = cercaDuplicati();
    const countEl = document.getElementById('duplicatiCount');
    const listEl = document.getElementById('ordiniDuplicatiList');
    
    if (countEl) {
        countEl.textContent = duplicati.length;
    }
        ...
        if (duplicati.length === 0) {
            listEl.innerHTML = `
                <div class="text-center py-4 text-muted">
                    ...
                    <p class="mb-0 mt-2">Nessun ordine duplicato</p>
                </div>
            `;
        } else {
            listEl.innerHTML = duplicati.map((gruppo, idx) => `
                <a href="#" class="list-group-item list-group-item-action" onclick="mostraGruppoDuplicati(${idx})">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>${gruppo[0].cliente}</strong>
                            <br><small class="text-muted">${gruppo[0].citta} - ${gruppo.length} ordini</small>
                        </div>
                        <div class="text-end">
                            <span class="badge bg-warning text-dark">${gruppo.length}</span>
                            <br><small class="text-success">€ ${gruppo.reduce((sum, o) => sum + o.contrassegno, 0).toFixed(2)}</small>
                        </div>
                    </div>
                </a>
            `).join('');
...
function apriGestioneDuplicati() {
    const modal = new bootstrap.Modal(document.getElementById('modalDuplicati'));
    modal.show();
    
    setTimeout(() => {
        renderDuplicatiModal();
    }, 300);
}

function renderDuplicatiModal() {
    const container = document.getElementById('duplicatiGruppiList');
    const duplicati = cercaDuplicati();
    ...
    if (duplicati.length === 0) {
        container.innerHTML = `
            <div class="text-center py-5">
                ...
                <h5 class="mt-3">Nessun ordine duplicato!</h5>
                <p class="text-muted">Tutti gli ordini sono unici.</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = duplicati.map((gruppo, gruppoIdx) => `
        <div class="card mb-3 border-warning">
            <div class="card-header bg-warning bg-opacity-25 d-flex justify-content-between align-items-center">
                <div>
                    <strong><i class="bi bi-person"></i> ${gruppo[0].cliente}</strong>
                    <span class="badge bg-dark ms-2">${gruppo.length} ordini</span>
                </div>
                <div>
                    <span class="text-success fw-bold">Totale: € ${gruppo.reduce((sum, o) => sum + o.contrassegno, 0).toFixed(2)}</span>
                    <button class="btn btn-sm btn-success ms-3" onclick="preparaUnificazione(${gruppoIdx})">
                        <i class="bi bi-link"></i> Unifica
                    </button>
                </div>
            </div>
            <div class="card-body p-0">
                <table class="table table-sm mb-0">
                    <thead class="bg-light">
                        <tr><th>Ordine</th><th>Data</th><th>Indirizzo</th><th>Prodotti</th><th>Contrassegno</th></tr>
                    </thead>
                    <tbody>
                        ${gruppo.map(ordine => `
                            <tr>
                                ...
                                <td>${ordine.data}</td>
                                <td><small>${ordine.indirizzo}, ${ordine.cap} ${ordine.citta}</small></td>
                                <td><small>${ordine.prodotti.join(', ')}</small></td>
                                <td><strong class="text-success">€ ${ordine.contrassegno.toFixed(2)}</strong></td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
function mostraGruppoDuplicati(gruppoIdx) {
    apriGestioneDuplicati();
}

function preparaUnificazione(gruppoIdx) {
    ordiniDaUnificare = ordiniDuplicati[gruppoIdx];
    
    // Chiudi modal duplicati
    bootstrap.Modal.getInstance(document.getElementById('modalDuplicati')).hide();
    
    // Popola modal unificazione
    const tbody = document.getElementById('ordiniDaUnificareBody');
    let totale = 0;
    
    tbody.innerHTML = ordiniDaUnificare.map(o => {
        totale += o.contrassegno;
        return `
            <tr>
                <td><strong>${o.id}</strong></td>
                <td>${o.cliente}<br><small class="text-muted">${o.indirizzo}, ${o.citta}</small></td>
                <td><small>${o.prodotti.join('<br>')}</small></td>
                <td><strong>€ ${o.contrassegno.toFixed(2)}</strong></td>
            </tr>
        `;
    }).join('');
    
    document.getElementById('totaleContrassegnoUnificato').textContent = '€ ' + totale.toFixed(2);
    document.getElementById('unificaColli').value = ordiniDaUnificare.length;
    document.getElementById('unificaPeso').value = (ordiniDaUnificare.length * 2).toFixed(1);
    document.getElementById('unificaNote').value = 'Ordini unificati: ' + ordiniDaUnificare.map(o => o.id).join(', ');
    
    // Apri modal unificazione
    setTimeout(() => {
        new bootstrap.Modal(document.getElementById('modalUnificaOrdini')).show();
    }, 300);
}

function eseguiUnificaOrdini() {
    const corriere = document.getElementById('unificaCorriere').value;
    const colli = parseInt(document.getElementById('unificaColli').value);
    const peso = parseFloat(document.getElementById('unificaPeso').value);
    const note = document.getElementById('unificaNote').value;
    
    if (!corriere) {
        showToast('Seleziona un corriere', 'warning');
        return;
    }
    
    // Calcola totale contrassegno
    const totaleContrassegno = ordiniDaUnificare.reduce((sum, o) => sum + o.contrassegno, 0);
    
    // Crea nuova spedizione unificata
    const nuovoId = Math.max(...spedizioni.map(s => s.id)) + 1;
    const nuovaSpedizione = {
        id: nuovoId,
        data: new Date().toLocaleDateString('it-IT'),
        corriere: corriere,
        destinatario: {
            nome: ordiniDaUnificare[0].cliente,
            indirizzo: ordiniDaUnificare[0].indirizzo,
            cap: ordiniDaUnificare[0].cap,
            citta: ordiniDaUnificare[0].citta,
            prov: '',
            telefono: ordiniDaUnificare[0].telefono,
            email: ordiniDaUnificare[0].email
        },
        dettagli: {
            colli: colli,
            peso: peso,
        ...
        stato: 'In Lavorazione',
        tracking: '',
        ordiniShopify: ordiniDaUnificare.map(o => o.id),
        prodottiUnificati: ordiniDaUnificare.flatMap(o => o.prodotti)
    };
    
    spedizioni.unshift(nuovaSpedizione);
    
    // Rimuovi ordini da Shopify (già processati)
    const idsProcessati = ordiniDaUnificare.map(o => o.id);
    ordiniShopify = ordiniShopify.filter(o => !idsProcessati.includes(o.id));
    
    // Chiudi modal
    bootstrap.Modal.getInstance(document.getElementById('modalUnificaOrdini')).hide();
    ...
    showToast(`Spedizione #${nuovoId} creata! ${ordiniDaUnificare.length} ordini unificati. Contrassegno totale: € ${totaleContrassegno.toFixed(2)}`, 'success');
}
...
// Override showPage per caricare dati specifici
const _originalShowPage = showPage;
showPage = function(pageId, filterType = null) {
    _originalShowPage(pageId, filterType);
    
    if (pageId === 'view-impostazioni') {
        // Carica utenti quando si apre impostazioni
        setTimeout(() => {
            loadUtenti();
            renderStoricoBackup();
        }, 100);
...
// Override loadDashboard per caricare duplicati
...
// Funzione showSettingsTab se non esiste
if (typeof showSettingsTab !== 'function') {
    window.showSettingsTab = function(tabName) {
        ...
        document.querySelectorAll('#view-impostazioni .list-group-item').forEach(item => item.classList.remove('active'));
        ...
        if (tabName === 'utenti') loadUtenti();
        if (tabName === 'backup') renderStoricoBackup();
    };
}
